...
type AddGroupAdUserHandler struct {
	app *App
}

func (h *AddGroupAdUserHandler) GetAccessRols() []string {
	return []string{RolAdmin, RolSpecialist}
}

func (h *AddGroupAdUserHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	var e error
	//Access Control
	...
	//Validating aduser not exists in group
	...
	if ok {
		h.app.elogger.Println(e)
		http.Error(w, "This group already have this user", http.StatusConflict)
		return
	}
	...
	//Add GroupAdUser to database
	e = h.app.datamodel.AddGroupAdUser(&group, newAdUser.AdUsername)
	if e != nil {
		h.app.elogger.Println(e)
		http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
		return
